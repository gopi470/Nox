...
- `src-tauri/` - **Rust Desktop Backend**
  ...
  - `src/bluetooth.rs` - Device status, connection monitoring (WASAPI/PnP hybrid), and native radio state telemetry.
  - `src/audio.rs` - WASAPI peak loudness metrics for global playback measurement.
  - `src/app_audio.rs` - Audio process attribution tracking and universal SMTC autopause logic.
...
* **Profiles Management**: `get_device_profiles` (fetch active profile and profiles list), `switch_active_profile` (change current active profile), `create_device_profile` (add a new custom earbud profile), `save_device_profile` (save changes to an existing profile), `delete_device_profile` (remove profile).
