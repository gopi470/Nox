...
mod tests {
    ...
    #[test]
    fn test_find_unused_name_basic() {
        let mut existing = HashSet::new();
        existing.insert("Buds");

        assert_eq!(find_unused_name("Buds", &existing), "Buds (2)");
        assert_eq!(find_unused_name("New Buds", &existing), "New Buds");
    }

    #[test]
    fn test_find_unused_name_multiple() {
        let mut existing = HashSet::new();
        existing.insert("Buds");
