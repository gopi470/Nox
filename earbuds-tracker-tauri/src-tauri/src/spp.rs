...
fn find_device_mac(device_name: &str) -> Option<u64> {
    ...
    let script = r#"
        $target = "*$($env:DEVICE_NAME)*"
        $dev = Get-PnpDevice -Class Bluetooth | Where-Object { $_.FriendlyName -like $target } | Select-Object -First 1;
...
fn read_battery_pnp(device_name: &str) -> Option<BatteryInfo> {
    ...
    let script = r#"
        $target = "*$($env:DEVICE_NAME)*"
        $devs = Get-PnpDevice | Where-Object { $_.FriendlyName -like $target };
        foreach ($d in $devs) {
