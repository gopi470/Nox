...
* 🔍 **Session Breakdown**:
  ...
  - **Export Formats**: Seamlessly download session summaries and app usage data in CSV or JSON.
* ⚙️ **Configurable Settings**: Custom playback goals, target device name mapping (with automatic paired-device discovery), desktop notification toggles, and adjustable battery polling intervals (from 30 seconds to 30 minutes).
* 🛡️ **Secure Data Purging**: Requires Windows account password authorization (via Windows Logon APIs) before allowing database resets.
...
3. **Node.js** & **Rust/Cargo toolchain** (for compiling)
